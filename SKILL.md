...
4. 进度节点模式：用户明确要求按百分比、阶段或里程碑提醒时，按用户指定的节点发送；没有明确要求时不发送中间进度通知。

## 发送

确认任务已达到完成、需决策、失败或取消等真实终态后，或已到达用户明确指定的进度节点时，生成一条独立可理解的通知，并调用 `scripts/bark_notify.sh`。进度通知应包含当前百分比、已完成内容、正在处理的内容和异常（如有）；不要把每个小步骤都当成进度节点。标题约 8–20 个中文字；正文通常包括结果、产物和建议。检查返回 JSON 中的 `code` 是否为 `200`，只在成功时报告已发送。
